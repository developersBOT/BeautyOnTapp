...
>
> **Addendum (28 Jul ~17:02Z, per T):** v8.1.7 also gained a **"Desktop logo width" control** in `sections/top-bar.liquid` (the desktop ≥1150px logo lives in the top bar — the header section's own logo is hidden at that width and was previously hardcoded to a 190×48px box). New range setting `desktop_logo_width` (150–300px, step 5, **default 190 = current size, zero visual change until moved**); logo box scales proportionally (~76px tall at 300px wide), the bar's fixed 80px height is untouched so there is no layout shift, and the logo source image is now requested at 2× the chosen width for retina sharpness. Upserted checksum verified (`c0d7da9aed7dce89ee174f6675e04a9b`); `assets/main.mjs` re-verified untouched (`7c465b4c…`). **v8.1.7 vs live is now exactly two files: `assets/main.mjs` + `sections/top-bar.liquid`.** Editor path: Customize (on the v8.1.7 preview) → Top bar section → "Desktop logo width".
